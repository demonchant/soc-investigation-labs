...
class ReportGenerator:
    def generate(self, findings, data):
        ...
        r = []
        r.append("="*68)
        r.append("  EXTERNAL ATTACK SURFACE MAPPER — REPORT")
        r.append("  Generated  : " + datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S UTC"))
        r.append("  Assets: {}  |  DNS Records: {}  |  Findings: {}".format(
            len(data.get("assets",[])), len(data.get("dns_records",[])), len(findings)))
        ...
        for i,f in enumerate(findings,1):
            ...
            r.append("  [{}] {} [{}] {}".format(i, lbl, f["asset"], f["title"]))
            r.append("       " + f["detail"])
