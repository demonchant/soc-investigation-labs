...
> Statistical + rule-based network flow analysis engine. Builds per-host behavioural baselines using mean/stddev profiling, then detects anomalies via Z-score deviation and 7 mapped to MITRE ATT&CK detection modules — all from stdlib only, no external dependencies.
