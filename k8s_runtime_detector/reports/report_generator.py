...
class ReportGenerator:
    def generate(self, findings, data):
        ...
        r = []
        r.append("="*68)
        r.append("  KUBERNETES RUNTIME THREAT DETECTOR — REPORT")
        r.append("  Generated  : " + datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S UTC"))
        r.append("  Pod Specs  : {}  |  Audit Events: {}  |  Findings: {}".format(
            len(data.get("pod_specs",[])), len(data.get("audit_log",[])), len(findings)))
        ...
        for i,f in enumerate(findings,1):
            ...
            r.append("  [{}] {} [{}] {}".format(i, lbl, f["resource"], f["title"]))
            r.append("       " + f["detail"])
