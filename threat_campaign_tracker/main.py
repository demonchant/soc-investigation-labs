...
from scanner.campaign_tracker import CampaignTracker
from reports.report_generator import ReportGenerator
logging.basicConfig(level=logging.INFO, format="%(levelname)s - %(message)s")

def run(data_file="data/incidents.json", output=None):
    print("[*] Threat Actor Campaign Tracker v1.0")
    with open(data_file) as f: data = json.load(f)
    findings = CampaignTracker().analyze_all(data)
    c = sum(1 for f in findings if f["severity"]=="critical")
    print("[+] {} correlation(s) — {} Critical.".format(len(findings), c))
    print()
...
if __name__ == "__main__":
    ...
    ap.add_argument("--output", default="reports/campaign_report.json")
    args = ap.parse_args()
