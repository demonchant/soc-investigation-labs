...
class ReportGenerator:
    def generate(self, findings, data):
        ...
        r = []
        r.append("="*68)
        r.append("  NETWORK BASELINE ANOMALY DETECTOR — REPORT")
        r.append("  Generated  : " + datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S UTC"))
        r.append("  Baseline Flows: {}  |  Current Flows: {}  |  Findings: {}".format(
            len(data.get("baseline_flows",[])), len(data.get("current_flows",[])), len(findings)))
        ...
        for i,f in enumerate(findings,1):
            ...
            r.append("  [{}] {} [{}] {}".format(i, lbl, f["host"], f["title"]))
            r.append("       " + f["detail"])
