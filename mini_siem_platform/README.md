...
> A lightweight but production-structured SIEM that ingests logs, stores them in SQLite, runs a multi-rule detection engine, generates mapped to MITRE ATT&CK alerts, and exposes a RESTful Flask API for SOC dashboard integration.
...
# Start API server
export SIEM_API_KEY="replace_with_a_long_random_value"
python api/server.py
```

When `SIEM_API_KEY` is set, protected endpoints accept the value through the
`X-API-Key` header or an `Authorization: Bearer` header. The health endpoint
remains public. Debug mode is disabled by default. Use `FLASK_DEBUG=true` only
...
* Real time streaming ingestion with Kafka or Redis
